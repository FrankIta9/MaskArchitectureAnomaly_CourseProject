            ...
            with torch.no_grad():
                for img_path in sampled_files:
                    try:
                        ...
                        # Verification log (temporary) - verify input shape before windowing
                        if img_tensor.dim() != 3 or img_tensor.shape[0] != 3:
                            logging.warning(f"⚠️ OOD validation: Unexpected img_tensor.shape = {img_tensor.shape} (expected [3, H, W])")
                        ...
                        # Verification log (temporary): window_imgs_semantic returns torch.stack(crops) = [B, 3, h, w]
                        if crops.dim() != 4 or crops.shape[1] != 3:
                            logging.warning(f"⚠️ OOD validation: Unexpected crops.shape = {crops.shape} (expected [B, 3, h, w])")
                        
                        # window_imgs_semantic returns torch.stack(crops) = [B, 3, h, w] (has batch dimension)
                        mask_logits_per_layer, class_logits_per_layer = self(crops)
